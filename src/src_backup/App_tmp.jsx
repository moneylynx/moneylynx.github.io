...
export default function App() {
  // ─── LOGIKA ZA DIJELJENJE, IZVOZ I UVOZ (BACKUP) ───
  const handleShare = async () => {
    const backupObj = { data, lists, prefs, user, sec, drafts };
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Moja lova Backup',
          text: `Podaci iz aplikacije (${data.length} transakcija).`,
          url: window.location.href
        });
      } catch (err) { console.log("Dijeljenje prekinuto."); }
    } else {
      alert("Vaš preglednik ne podržava direktno dijeljenje.");
    }
  };

  const handleExport = (format) => {
    const backupObj = { data, lists, prefs, user, sec, drafts, timestamp: new Date().toISOString() };
    let content, type, filename;

    if (format === 'json') {
      content = JSON.stringify(backupObj, null, 2);
      type = 'application/json';
      filename = `moja-lova-backup-${new Date().toISOString().slice(0,10)}.json`;
    } else {
      const headers = "Datum,Opis,Iznos,Kategorija,Lokacija,Plaćanje,Status\n";
      const rows = data.map(i => `${i.date},"${i.desc}",${i.amt},"${i.cat}","${i.loc}","${i.pay}","${i.st}"`).join("\n");
      content = headers + rows;
      type = 'text/csv;charset=utf-8;';
      filename = `moja-lova-izvoz-${new Date().toISOString().slice(0,10)}.csv`;
    }
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (imp.lists) setLists(imp.lists);
        if (imp.prefs) setPrefs({ ...imp.prefs, onboard: true });
        if (imp.user) setUser(imp.user);
        if (imp.sec) setSec(imp.sec);
        if (imp.drafts) setDrafts(imp.drafts);
        alert("Podaci su uspješno uvezeni! Aplikacija se osvježava...");
        window.location.reload();
      } catch (err) {
        alert("Greška: Datoteka nije ispravna JSON sigurnosna kopija.");
      }
    };
