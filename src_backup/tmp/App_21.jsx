...
export default function App() {
  ...
  return (
    <div style={{ ...wrap, paddingBottom:88 }}>
      ...
      {page==="recurring"    && <RecurringScreen {...shared} data={txs} setTxs={setTxs} onBack={()=>setPage("add")}/>}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
          ...
          {inst > 1 && (
             <div style={{ fontSize:11, color:C.textMuted, marginTop:-5, paddingLeft:4, display:"flex", alignItems:"center", gap:5 }}>
                <Ic n="info" s={12} c={C.textMuted}/> Status rata određuje se automatski.
             </div>
          )}
...
function TxList({ C, data, year, onEdit, onDelete, onDeleteGroup, onPay }) {
  ...
  const rows = useMemo(()=>{
    ...
    if (filter==="pending") f = f.filter(t=>t.status==="Čeka plaćanje");
    if (filter==="processing") f = f.filter(t=>t.status==="U obradi");
    if (q) f = f.filter(t=>t.description?.toLowerCase().includes(q.toLowerCase())||t.category?.toLowerCase().includes(q.toLowerCase()));
    
    // Čeka plaćanje i U obradi idu najstariji prvi, ostalo najnoviji
    ...
    <div className="fi" style={{ width:"100%" }}>
      ...
      <div style={{ padding:"12px 16px 0" }}>
        ...
        <div style={{ display:"flex", gap:6, marginBottom:12, overflowX:"auto", paddingBottom:4 }}>
          {[["all","Sve"],["income","Primici"],["expense","Troškovi"],["pending","Čeka plaćanje"],["processing","U obradi"]].map(([id,lb])=>(
            <Pill key={id} label={lb} active={filter===id} color={id==="pending"?C.warning:id==="processing"?"#FB923C":C.accent} onClick={()=>setFilter(id)}/>
          ))}
        </div>
            ...
            <div key={tx.id} className="su" style={{ background:C.card, border:`1px solid ${C.border}`, borderLeft:`3px solid ${tx.installmentGroup?C.warning:tx.type==="Primitak"?C.income:C.expense}`, borderRadius:14, padding:13, marginBottom:8, animationDelay:`${i*.02}s` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"stretch" }}>
                <div style={{ flex:1, minWidth:0, textAlign:"left" }}>
                  ...
                  <div style={{ fontSize:11, color:C.textMuted, marginTop:2 }}>{tx.payment} · <span style={{ color:tx.status==="Plaćeno"?C.income:tx.status==="U obradi"?"#FB923C":C.warning }}>{tx.status==="Plaćeno" ? "Plaćeno" : tx.status==="Čeka plaćanje" ? "Čeka plaćanje" : tx.status}</span></div>
                </div>
                
                {/* CSS Trik: flexDirection:"column" i marginTop:"auto" za savršeno poravnanje */}
...
function Charts({ C, data, year, lists }) {
  ...
  const [expFilter, setExpFilter] = useState({recurring:true, rate:true, kredit:true, processing:true});
  ...
  const mBar  = useMemo(()=>MONTHS.map((m,mi)=>{
    ...
    return{
      ...
      Čeka:mt.filter(t=>t.status==="Čeka plaćanje").reduce((s,t)=>s+(+t.amount||0),0),
      Obrada:mt.filter(t=>t.status==="U obradi").reduce((s,t)=>s+(+t.amount||0),0),
      Obveze:recAmt,
    };
  }),[yd,rec]);
    ...
    if (expFilter.processing) {
      fd.filter(t=>(t.status==="Čeka plaćanje" || t.status==="U obradi") && !t.installmentGroup && !t.recurringId)
        .forEach(t=>items.push({
            type: t.status === "U obradi" ? "processing" : "pending", 
            desc: t.description, 
            amount: +t.amount||0, 
            date: t.date, 
            cat: t.category
  ...
  const typeLabel = {rata:"Obrok", obveza:"Obveza", pending:"Čeka", processing:"Obrada", kredit:"Rata"};
          ...
          <div style={chartCard}>
            ...
            <div style={{ display:"flex", gap:6, marginBottom:12, flexWrap:"wrap" }}>
              {[
                ...
                {k:"processing",lb:"Ostalo",col:"#FB923C"} 
              ].map(({k,lb,col})=>(
        ...
        {tab==="overview" && <>
          <div style={chartCard}>
            ...
            <BarChart width={W} height={260} data={mBar}><CartesianGrid strokeDasharray="3 3" stroke={C.border}/><XAxis dataKey="name" tick={{fill:C.textMuted,fontSize:9}} axisLine={false}/><YAxis tick={{fill:C.textMuted,fontSize:9}} axisLine={false} width={44} tickFormatter={v=>v>=1000?`${(v/1000).toFixed(0)}k`:v}/><Tooltip {...tt}/><Legend wrapperStyle={{fontSize:10}}/>
              <Bar dataKey="Primici" fill={C.income} radius={[3,3,0,0]}/>
              <Bar dataKey="Troškovi" fill={C.expense} radius={[3,3,0,0]}/>
              <Bar dataKey="Čeka" fill={C.warning} radius={[3,3,0,0]}/>
              <Bar dataKey="Obrada" fill="#FB923C" radius={[3,3,0,0]}/>
              <Bar dataKey="Obveze" fill={`${C.accent}80`} radius={[3,3,0,0]}/>
            </BarChart>
          </div>
          <div style={chartCard}>
...
// ─── RecurringScreen (Redovne obveze & Mjesečni checklist) ────────────────────
function RecurringScreen({ C, lists, data, setTxs, onBack }) {
  const cm   = curMonth();
  const cms  = sm(cm);
  const now  = new Date();
  ...
  // Spoji Redovne obveze (Templates), Obroke i druge jednokratne troškove za ovaj mjesec
  const items = useMemo(() => {
    const list = [];

    // --- A) Redovne obveze (Templates) ---
    const rec = lists.recurring || [];
    const recTxsThisMonth = data.filter(t => {
      const d = new Date(t.date);
      return d.getMonth()===cmIdx && d.getFullYear()===cmYr && t.recurringId;
    });
    const paidMap = {};
    recTxsThisMonth.forEach(t => paidMap[t.recurringId] = t);
    ...
    // --- B) Obroci (Installments) koji padaju u ovaj mjesec ---
    const instThisMonth = data.filter(t => {
      const d = new Date(t.date);
      return d.getMonth()===cmIdx && d.getFullYear()===cmYr && t.installmentGroup;
    });
    ...
    // --- C) Ostali jednokratni troškovi koji čekaju (samo za tekući mjesec) ---
    const otherPendingThisMonth = data.filter(t => {
        const d = new Date(t.date);
        return d.getMonth()===cmIdx && d.getFullYear()===cmYr && (t.status === "Čeka plaćanje" || t.status === "U obradi") && !t.installmentGroup && !t.recurringId;
    });
  ...
  const togglePay = (item) => {
    if (item.isPaid) {
      if (item.type === "recurring") {
        setTxs(p => p.filter(t => t.id !== item.txId));
      } else if (item.type === "installment") {
        setTxs(p => p.map(t => t.id === item.txId ? {...t, status: "Čeka plaćanje"} : t));
      }
    } else {
      if (item.type === "recurring") {
        const newTx = {
          ...
          installments: 0,
          recurringId: item.template.id,
        };
        setTxs(p => [...p, newTx]);
      } else if (item.type === "installment" || item.type === "other") {
        setTxs(p => p.map(t => t.id === item.txId ? {...t, status: "Plaćeno", date: now.toISOString().split("T")[0]} : t));
      }
    }
  };
      ...
      <div style={{ padding:"14px 16px 0" }}>
        ...
        {items.length === 0 ? (
          <div style={{ textAlign:"center", padding:"50px 20px", color:C.textMuted }}>
            ...
            <p style={{ fontSize:14, fontWeight:600, marginBottom:6 }}>Nema obveza za ovaj mjesec</p>
            <p style={{ fontSize:12 }}>Ovdje će se pojaviti redovne obveze i obroci čiji datum pada u tekući mjesec.</p>
          </div>
        ) : items.map((item, i) => {
          return (
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div style={{ flex:1, minWidth:0, textAlign:"left" }}>
                  ...
                  <div style={{ fontSize:11, color:C.textMuted, marginTop:4, paddingLeft:22 }}>
                    ...
                    {item.type==="recurring" && item.dueDate && <span> · dospijeva {item.dueDate}. u mj.</span>}
                    {item.type==="installment" && <span> · Obrok</span>}
                    {item.location && <span> · {item.location}</span>}
                    {item.type==="recurring" && item.totalPayments>0 && <span> · {item.totalPayments} obroka ukupno</span>}
                  </div>
                </div>
                <div style={{ textAlign:"right", flexShrink:0, marginLeft:10, display:"flex", flexDirection:"column", alignItems:"flex-end" }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginTop:8 }}>
                    <span style={{ fontSize:11, fontWeight:600, color:item.isPaid?C.income:C.warning }}>
                        {item.isPaid ? "Plaćeno" : "Čeka plaćanje"}
                    </span>
                    <button onClick={()=>togglePay(item)}
                        ...
                        {item.isPaid ? <><Ic n="arrow_l" s={11} c={C.textMuted}/> Vrati</> : <><Ic n="check" s={11} c="#fff"/> Plati</>}
                    </button>
                  </div>
        ...
        {items.length > 0 && (
          <div style={{ textAlign:"center", padding:"12px 0 20px", color:C.textMuted, fontSize:12 }}>
            {paidCount === items.length
              ...
              : `${items.length - paidCount} od ${items.length} obveza čeka plaćanje`
            }
          </div>
        )}
      </div>
